...
<div id="table-container">
    <table id="studentTable"
        ...
        %>
        <tr>
            <td style="text-align: center; padding: 10px"><%= s.getFirstName() %>
            </td>
            <td style="text-align: center; padding: 10px"><%= s.getLastName() %>
            </td>
            <td style="display: none;"><%= s.getEmail()%>
            </td>
            <td style="text-align: center; padding: 10px"><%= s.getGender() %>
            </td>
            <td style="text-align: center; padding: 10px"><%= age %>
            </td>
            <td style="text-align: center; padding: 10px"><%= s.getClassName() %>
            </td>
    ...
    // if no student found then showing this message
    <div id="noResultMsg"
         style="display:none; color:red; margin-top:25px; font-weight:bold; padding-left: 700px; font-size: 20px">
        No student found
    </div>
</div>
...
<script>
    ...
    function paginateTable() {
        ...
        // Note: We also check if the row matches the search filter (if search is active)
        // For simplicity here, this basic pagination assumes no active search filtering conflict.
    ...
    // ... (Logic to create Prev, 1, 2, 3, Next buttons) ...
    ...
    function searchTable() {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (filterType === "fname") {
                ...
                match = gender.includes(input);
            } else { // "All" filter: check any column
                match =
                    firstName.includes(input) ||
                    lastName.includes(input) ||
                    email.includes(input) ||
